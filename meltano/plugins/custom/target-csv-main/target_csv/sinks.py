...
class CSVSink(BatchSink):
    ...
    @property
    def timestamp_time(self) -> datetime: # noqa: D102
        if not self._timestamp_time:
            self._timestamp_time = datetime.now(
    ...
    def output_file(self) -> Path:  # noqa: D102
        ...
        user_defined_path = self.config.get(
            "output_path",
            self.config.get(
        ...
        if user_defined_path is None:
            raise ValueError("Nenhum caminho de saída foi definido na configuração.")

        data_hoje = datetime.today().strftime('%Y-%m-%d')

        final_output_path = Path(user_defined_path) / data_hoje

        try:
            final_output_path.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            warnings.warn(f"Não foi possível criar o diretório {final_output_path}: {e}", category=UserWarning)
